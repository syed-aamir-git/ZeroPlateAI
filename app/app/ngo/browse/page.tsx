        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between text-xs text-stone-600 bg-white p-4 rounded-2xl border border-stone-200/90 shadow-xs flex-wrap gap-2">
            ...
            <span className="font-mono font-bold text-emerald-800 bg-emerald-50 px-2.5 py-1 rounded-lg border border-emerald-200">
              {filteredListings.length} Active Listings Mapped
            </span>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {filteredListings.map((item) => {
            ...
            return (
              <div
                key={item._id}
                className="rounded-2xl bg-white border border-stone-200/90 p-5 sm:p-6 shadow-xs hover:shadow-md hover:border-emerald-300 transition-all flex flex-col justify-between space-y-5 text-left group"
              >
                <div>
                  {/* Category Stamp and Verified Badge */}
                  ...
                  <div className="space-y-1.5">
                    <h3 className="font-serif font-bold text-lg text-stone-900 group-hover:text-emerald-800 transition-colors line-clamp-2">
                      {item.itemName}
                    </h3>
                    <div className="flex items-baseline gap-2">
                      ...
                      <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold bg-emerald-50 text-emerald-800 border border-emerald-200">
                        ~{Math.round(item.quantity * 2.5)} meals
                      </span>
                    </div>
                  </div>
                ...
                <div className="pt-3 border-t border-stone-100">
                  {isKycApproved ? (
                    <button
                      ...
                      disabled={isClaiming}
                      className="w-full inline-flex items-center justify-center gap-2 h-10 px-4 rounded-xl bg-emerald-700 hover:bg-emerald-800 active:scale-[0.98] disabled:opacity-60 text-white text-xs font-semibold shadow-xs hover:shadow-md transition-all cursor-pointer"
                    >
                      {isClaiming ? (
                        <>
